...
type assetService interface {
	...
	BuildVeX(asset models.Asset, version, orgName string, components []models.ComponentDependency, dependencyVulns []models.DependencyVulnerability) *cdx.BOM
}
...
func NewHttpController(repository repository, assetComponentsLoader assetComponentsLoader, dependencyVulnRepository dependencyVulnRepository, assetService assetService, supplyChainRepository supplyChainRepository) *httpController {
...
func (a *httpController) AffectedComponents(c core.Context) error {
	...
	asset := core.GetAsset(c)
	_, dependencyVulns, err := a.getComponentsAndDependencyVulns(asset, scanner, version)
	if err != nil {
		return err
	}

	return c.JSON(200, utils.Map(dependencyVulns, func(m models.DependencyVulnerability) DependencyVuln.DependencyVulnDTO {
		return DependencyVuln.DependencyVulnToDto(m)
	}))
}

func (a *httpController) getComponentsAndDependencyVulns(asset models.Asset, scanner, version string) ([]models.ComponentDependency, []models.DependencyVulnerability, error) {
...
func (a *httpController) Metrics(c core.Context) error {
	...
	err := a.assetRepository.GetDB(nil).Table("dependencyVulns").Select("DISTINCT scanner_id").Where("asset_id  = ?", asset.ID).Pluck("scanner_id", &scannerIds).Error
...
func (a *httpController) buildVeX(c core.Context) (*cdx.BOM, error) {
	...
	components, dependencyVulns, err := a.getComponentsAndDependencyVulns(asset, scanner, version)
	if err != nil {
		return nil, err
	}

	return a.assetService.BuildVeX(asset, version, org.Name, components, dependencyVulns), nil
}
