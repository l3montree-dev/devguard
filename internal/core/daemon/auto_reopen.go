...
func AutoReopenAcceptedVulnerabilities(db core.DB) error {
	...
	for _, asset := range assets {
		...
		for _, vuln := range vulnerabilities {
			...
			event := models.NewReopenedEvent(vuln.ID, models.VulnTypeDependencyVuln, "system", fmt.Sprintf("Automatically reopened since the vulnerability was accepted more than %d days ago", *asset.VulnAutoReopenAfterDays), 0)
