...
func TestServeCSAFReportRequest(t *testing.T) {
	...
	csafController := NewCSAFController(dependencyVulnRepository, vulnEventRepository, assetVersionRepository, repositories.NewAssetRepository(db), repositories.NewProjectRepository(db), repositories.NewOrgRepository(db))
	// Setup echo app
	app := echo.New()
	...
	t.Run("test product_tree functionality for more complex product trees", func(t *testing.T) {
		...
		assert.Equal(t, "v1", csafDoc.ProductTree.Branches[1].Name)
		assert.Equal(t, "product_version", csafDoc.ProductTree.Branches[1].Category)
		assert.Equal(t, "v1", csafDoc.ProductTree.Branches[1].Product.ProductID)
		assert.Equal(t, "v1", csafDoc.ProductTree.Branches[1].Product.Name)

		assert.Equal(t, "v2", csafDoc.ProductTree.Branches[2].Name)
		assert.Equal(t, "product_version", csafDoc.ProductTree.Branches[2].Category)
		assert.Equal(t, "v2", csafDoc.ProductTree.Branches[2].Product.ProductID)
		assert.Equal(t, "v2", csafDoc.ProductTree.Branches[2].Product.Name)
	})
	t.Run("add vulnerabilities and vuln events to simulate a vulnerability history, use latest report version", func(t *testing.T) {
		...
		err = repositories.NewDependencyVulnRepository(db).SaveBatch(nil, []models.DependencyVuln{vuln1, vuln2, vuln3, vuln4, vuln5, vulnExtra})
		if err != nil {
			t.Fail()
		}
		...
		err = vulnEventRepository.SaveBatch(nil, []models.VulnEvent{event1, event2, event3, event4, event5, event6, event7, event8, event9, event10, eventExtra})
		if err != nil {
			t.Fail()
		}
	...
	t.Run("use an earlier version to test the time travel functionality", func(t *testing.T) {
		...
		assert.Equal(t, "CVE-2025-22871", csafDoc.Vulnerabilities[1].CVE, csafDoc.Vulnerabilities[1].Title)
		assert.Equal(t, "Version main: unhandled for purl pkg:golang/helm.sh/helm/v3@v3.18.4", csafDoc.Vulnerabilities[1].Notes[0].Text)
		assert.Equal(t, "main", csafDoc.Vulnerabilities[1].ProductStatus.KnownAffected[0])

		assert.Equal(t, "CVE-2025-22777", csafDoc.Vulnerabilities[2].CVE, csafDoc.Vulnerabilities[2].Title)
		assert.Equal(t, "Version v1: unhandled for purl pkg:golang/github.com/ulikunitz/xz@v0.5.12| Version v2: unhandled for purl pkg:golang/stdlib@v1.24.1", csafDoc.Vulnerabilities[2].Notes[0].Text)
		assert.Equal(t, "v1", csafDoc.Vulnerabilities[2].ProductStatus.KnownAffected[0])
		assert.Equal(t, "v2", csafDoc.Vulnerabilities[2].ProductStatus.KnownAffected[1])
	})
}
