...
func generateProductTree(asset models.Asset, assetVersionRepository core.AssetVersionRepository, artifactRepository core.ArtifactRepository, vulnsForCVE []models.DependencyVuln) (gocsaf.ProductTree, error) {
...
func generateNotesForVulnerabilityObject(vulns []models.DependencyVuln) ([]*gocsaf.Note, error) {
	...
	for artifact, vulns := range artifactToVulns {
		...
		for _, vuln := range vulns {
			...
			if recentJustification != nil && eventType != models.EventTypeMitigate {
