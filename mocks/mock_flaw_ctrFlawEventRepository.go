// Code generated by mockery v2.46.2. DO NOT EDIT.

package mocks

import (
	core "github.com/l3montree-dev/devguard/internal/core"

	mock "github.com/stretchr/testify/mock"

	models "github.com/l3montree-dev/devguard/internal/database/models"
)

// FlawCtrFlawEventRepository is an autogenerated mock type for the ctrFlawEventRepository type
type FlawCtrFlawEventRepository struct {
	mock.Mock
}

type FlawCtrFlawEventRepository_Expecter struct {
	mock *mock.Mock
}

func (_m *FlawCtrFlawEventRepository) EXPECT() *FlawCtrFlawEventRepository_Expecter {
	return &FlawCtrFlawEventRepository_Expecter{mock: &_m.Mock}
}

// Create provides a mock function with given fields: tx, flawEvent
func (_m *FlawCtrFlawEventRepository) Create(tx core.DB, flawEvent *models.FlawEvent) error {
	ret := _m.Called(tx, flawEvent)

	if len(ret) == 0 {
		panic("no return value specified for Create")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(core.DB, *models.FlawEvent) error); ok {
		r0 = rf(tx, flawEvent)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// FlawCtrFlawEventRepository_Create_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Create'
type FlawCtrFlawEventRepository_Create_Call struct {
	*mock.Call
}

// Create is a helper method to define mock.On call
//   - tx core.DB
//   - flawEvent *models.FlawEvent
func (_e *FlawCtrFlawEventRepository_Expecter) Create(tx interface{}, flawEvent interface{}) *FlawCtrFlawEventRepository_Create_Call {
	return &FlawCtrFlawEventRepository_Create_Call{Call: _e.mock.On("Create", tx, flawEvent)}
}

func (_c *FlawCtrFlawEventRepository_Create_Call) Run(run func(tx core.DB, flawEvent *models.FlawEvent)) *FlawCtrFlawEventRepository_Create_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(core.DB), args[1].(*models.FlawEvent))
	})
	return _c
}

func (_c *FlawCtrFlawEventRepository_Create_Call) Return(_a0 error) *FlawCtrFlawEventRepository_Create_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *FlawCtrFlawEventRepository_Create_Call) RunAndReturn(run func(core.DB, *models.FlawEvent) error) *FlawCtrFlawEventRepository_Create_Call {
	_c.Call.Return(run)
	return _c
}

// NewFlawCtrFlawEventRepository creates a new instance of FlawCtrFlawEventRepository. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewFlawCtrFlawEventRepository(t interface {
	mock.TestingT
	Cleanup(func())
}) *FlawCtrFlawEventRepository {
	mock := &FlawCtrFlawEventRepository{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
