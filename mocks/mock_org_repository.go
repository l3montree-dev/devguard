// Code generated by mockery v2.46.2. DO NOT EDIT.

package mocks

import (
	core "github.com/l3montree-dev/devguard/internal/core"
	mock "github.com/stretchr/testify/mock"

	models "github.com/l3montree-dev/devguard/internal/database/models"

	obj "github.com/l3montree-dev/devguard/internal/obj"

	uuid "github.com/google/uuid"
)

// OrgRepository is an autogenerated mock type for the repository type
type OrgRepository struct {
	mock.Mock
}

type OrgRepository_Expecter struct {
	mock *mock.Mock
}

func (_m *OrgRepository) EXPECT() *OrgRepository_Expecter {
	return &OrgRepository_Expecter{mock: &_m.Mock}
}

// Begin provides a mock function with given fields:
func (_m *OrgRepository) Begin() core.DB {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for Begin")
	}

	var r0 core.DB
	if rf, ok := ret.Get(0).(func() core.DB); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(core.DB)
		}
	}

	return r0
}

// OrgRepository_Begin_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Begin'
type OrgRepository_Begin_Call struct {
	*mock.Call
}

// Begin is a helper method to define mock.On call
func (_e *OrgRepository_Expecter) Begin() *OrgRepository_Begin_Call {
	return &OrgRepository_Begin_Call{Call: _e.mock.On("Begin")}
}

func (_c *OrgRepository_Begin_Call) Run(run func()) *OrgRepository_Begin_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *OrgRepository_Begin_Call) Return(_a0 core.DB) *OrgRepository_Begin_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *OrgRepository_Begin_Call) RunAndReturn(run func() core.DB) *OrgRepository_Begin_Call {
	_c.Call.Return(run)
	return _c
}

// ContentTree provides a mock function with given fields: orgID, projects
func (_m *OrgRepository) ContentTree(orgID uuid.UUID, projects []string) []obj.ContentTreeElement {
	ret := _m.Called(orgID, projects)

	if len(ret) == 0 {
		panic("no return value specified for ContentTree")
	}

	var r0 []obj.ContentTreeElement
	if rf, ok := ret.Get(0).(func(uuid.UUID, []string) []obj.ContentTreeElement); ok {
		r0 = rf(orgID, projects)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]obj.ContentTreeElement)
		}
	}

	return r0
}

// OrgRepository_ContentTree_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'ContentTree'
type OrgRepository_ContentTree_Call struct {
	*mock.Call
}

// ContentTree is a helper method to define mock.On call
//   - orgID uuid.UUID
//   - projects []string
func (_e *OrgRepository_Expecter) ContentTree(orgID interface{}, projects interface{}) *OrgRepository_ContentTree_Call {
	return &OrgRepository_ContentTree_Call{Call: _e.mock.On("ContentTree", orgID, projects)}
}

func (_c *OrgRepository_ContentTree_Call) Run(run func(orgID uuid.UUID, projects []string)) *OrgRepository_ContentTree_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(uuid.UUID), args[1].([]string))
	})
	return _c
}

func (_c *OrgRepository_ContentTree_Call) Return(_a0 []obj.ContentTreeElement) *OrgRepository_ContentTree_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *OrgRepository_ContentTree_Call) RunAndReturn(run func(uuid.UUID, []string) []obj.ContentTreeElement) *OrgRepository_ContentTree_Call {
	_c.Call.Return(run)
	return _c
}

// Create provides a mock function with given fields: tx, t
func (_m *OrgRepository) Create(tx core.DB, t *models.Org) error {
	ret := _m.Called(tx, t)

	if len(ret) == 0 {
		panic("no return value specified for Create")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(core.DB, *models.Org) error); ok {
		r0 = rf(tx, t)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// OrgRepository_Create_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Create'
type OrgRepository_Create_Call struct {
	*mock.Call
}

// Create is a helper method to define mock.On call
//   - tx core.DB
//   - t *models.Org
func (_e *OrgRepository_Expecter) Create(tx interface{}, t interface{}) *OrgRepository_Create_Call {
	return &OrgRepository_Create_Call{Call: _e.mock.On("Create", tx, t)}
}

func (_c *OrgRepository_Create_Call) Run(run func(tx core.DB, t *models.Org)) *OrgRepository_Create_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(core.DB), args[1].(*models.Org))
	})
	return _c
}

func (_c *OrgRepository_Create_Call) Return(_a0 error) *OrgRepository_Create_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *OrgRepository_Create_Call) RunAndReturn(run func(core.DB, *models.Org) error) *OrgRepository_Create_Call {
	_c.Call.Return(run)
	return _c
}

// CreateBatch provides a mock function with given fields: tx, ts
func (_m *OrgRepository) CreateBatch(tx core.DB, ts []models.Org) error {
	ret := _m.Called(tx, ts)

	if len(ret) == 0 {
		panic("no return value specified for CreateBatch")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(core.DB, []models.Org) error); ok {
		r0 = rf(tx, ts)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// OrgRepository_CreateBatch_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'CreateBatch'
type OrgRepository_CreateBatch_Call struct {
	*mock.Call
}

// CreateBatch is a helper method to define mock.On call
//   - tx core.DB
//   - ts []models.Org
func (_e *OrgRepository_Expecter) CreateBatch(tx interface{}, ts interface{}) *OrgRepository_CreateBatch_Call {
	return &OrgRepository_CreateBatch_Call{Call: _e.mock.On("CreateBatch", tx, ts)}
}

func (_c *OrgRepository_CreateBatch_Call) Run(run func(tx core.DB, ts []models.Org)) *OrgRepository_CreateBatch_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(core.DB), args[1].([]models.Org))
	})
	return _c
}

func (_c *OrgRepository_CreateBatch_Call) Return(_a0 error) *OrgRepository_CreateBatch_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *OrgRepository_CreateBatch_Call) RunAndReturn(run func(core.DB, []models.Org) error) *OrgRepository_CreateBatch_Call {
	_c.Call.Return(run)
	return _c
}

// Delete provides a mock function with given fields: tx, id
func (_m *OrgRepository) Delete(tx core.DB, id uuid.UUID) error {
	ret := _m.Called(tx, id)

	if len(ret) == 0 {
		panic("no return value specified for Delete")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(core.DB, uuid.UUID) error); ok {
		r0 = rf(tx, id)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// OrgRepository_Delete_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Delete'
type OrgRepository_Delete_Call struct {
	*mock.Call
}

// Delete is a helper method to define mock.On call
//   - tx core.DB
//   - id uuid.UUID
func (_e *OrgRepository_Expecter) Delete(tx interface{}, id interface{}) *OrgRepository_Delete_Call {
	return &OrgRepository_Delete_Call{Call: _e.mock.On("Delete", tx, id)}
}

func (_c *OrgRepository_Delete_Call) Run(run func(tx core.DB, id uuid.UUID)) *OrgRepository_Delete_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(core.DB), args[1].(uuid.UUID))
	})
	return _c
}

func (_c *OrgRepository_Delete_Call) Return(_a0 error) *OrgRepository_Delete_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *OrgRepository_Delete_Call) RunAndReturn(run func(core.DB, uuid.UUID) error) *OrgRepository_Delete_Call {
	_c.Call.Return(run)
	return _c
}

// GetDB provides a mock function with given fields: tx
func (_m *OrgRepository) GetDB(tx core.DB) core.DB {
	ret := _m.Called(tx)

	if len(ret) == 0 {
		panic("no return value specified for GetDB")
	}

	var r0 core.DB
	if rf, ok := ret.Get(0).(func(core.DB) core.DB); ok {
		r0 = rf(tx)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(core.DB)
		}
	}

	return r0
}

// OrgRepository_GetDB_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetDB'
type OrgRepository_GetDB_Call struct {
	*mock.Call
}

// GetDB is a helper method to define mock.On call
//   - tx core.DB
func (_e *OrgRepository_Expecter) GetDB(tx interface{}) *OrgRepository_GetDB_Call {
	return &OrgRepository_GetDB_Call{Call: _e.mock.On("GetDB", tx)}
}

func (_c *OrgRepository_GetDB_Call) Run(run func(tx core.DB)) *OrgRepository_GetDB_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(core.DB))
	})
	return _c
}

func (_c *OrgRepository_GetDB_Call) Return(_a0 core.DB) *OrgRepository_GetDB_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *OrgRepository_GetDB_Call) RunAndReturn(run func(core.DB) core.DB) *OrgRepository_GetDB_Call {
	_c.Call.Return(run)
	return _c
}

// List provides a mock function with given fields: ids
func (_m *OrgRepository) List(ids []uuid.UUID) ([]models.Org, error) {
	ret := _m.Called(ids)

	if len(ret) == 0 {
		panic("no return value specified for List")
	}

	var r0 []models.Org
	var r1 error
	if rf, ok := ret.Get(0).(func([]uuid.UUID) ([]models.Org, error)); ok {
		return rf(ids)
	}
	if rf, ok := ret.Get(0).(func([]uuid.UUID) []models.Org); ok {
		r0 = rf(ids)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]models.Org)
		}
	}

	if rf, ok := ret.Get(1).(func([]uuid.UUID) error); ok {
		r1 = rf(ids)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// OrgRepository_List_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'List'
type OrgRepository_List_Call struct {
	*mock.Call
}

// List is a helper method to define mock.On call
//   - ids []uuid.UUID
func (_e *OrgRepository_Expecter) List(ids interface{}) *OrgRepository_List_Call {
	return &OrgRepository_List_Call{Call: _e.mock.On("List", ids)}
}

func (_c *OrgRepository_List_Call) Run(run func(ids []uuid.UUID)) *OrgRepository_List_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].([]uuid.UUID))
	})
	return _c
}

func (_c *OrgRepository_List_Call) Return(_a0 []models.Org, _a1 error) *OrgRepository_List_Call {
	_c.Call.Return(_a0, _a1)
	return _c
}

func (_c *OrgRepository_List_Call) RunAndReturn(run func([]uuid.UUID) ([]models.Org, error)) *OrgRepository_List_Call {
	_c.Call.Return(run)
	return _c
}

// Read provides a mock function with given fields: id
func (_m *OrgRepository) Read(id uuid.UUID) (models.Org, error) {
	ret := _m.Called(id)

	if len(ret) == 0 {
		panic("no return value specified for Read")
	}

	var r0 models.Org
	var r1 error
	if rf, ok := ret.Get(0).(func(uuid.UUID) (models.Org, error)); ok {
		return rf(id)
	}
	if rf, ok := ret.Get(0).(func(uuid.UUID) models.Org); ok {
		r0 = rf(id)
	} else {
		r0 = ret.Get(0).(models.Org)
	}

	if rf, ok := ret.Get(1).(func(uuid.UUID) error); ok {
		r1 = rf(id)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// OrgRepository_Read_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Read'
type OrgRepository_Read_Call struct {
	*mock.Call
}

// Read is a helper method to define mock.On call
//   - id uuid.UUID
func (_e *OrgRepository_Expecter) Read(id interface{}) *OrgRepository_Read_Call {
	return &OrgRepository_Read_Call{Call: _e.mock.On("Read", id)}
}

func (_c *OrgRepository_Read_Call) Run(run func(id uuid.UUID)) *OrgRepository_Read_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(uuid.UUID))
	})
	return _c
}

func (_c *OrgRepository_Read_Call) Return(_a0 models.Org, _a1 error) *OrgRepository_Read_Call {
	_c.Call.Return(_a0, _a1)
	return _c
}

func (_c *OrgRepository_Read_Call) RunAndReturn(run func(uuid.UUID) (models.Org, error)) *OrgRepository_Read_Call {
	_c.Call.Return(run)
	return _c
}

// ReadBySlug provides a mock function with given fields: slug
func (_m *OrgRepository) ReadBySlug(slug string) (models.Org, error) {
	ret := _m.Called(slug)

	if len(ret) == 0 {
		panic("no return value specified for ReadBySlug")
	}

	var r0 models.Org
	var r1 error
	if rf, ok := ret.Get(0).(func(string) (models.Org, error)); ok {
		return rf(slug)
	}
	if rf, ok := ret.Get(0).(func(string) models.Org); ok {
		r0 = rf(slug)
	} else {
		r0 = ret.Get(0).(models.Org)
	}

	if rf, ok := ret.Get(1).(func(string) error); ok {
		r1 = rf(slug)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// OrgRepository_ReadBySlug_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'ReadBySlug'
type OrgRepository_ReadBySlug_Call struct {
	*mock.Call
}

// ReadBySlug is a helper method to define mock.On call
//   - slug string
func (_e *OrgRepository_Expecter) ReadBySlug(slug interface{}) *OrgRepository_ReadBySlug_Call {
	return &OrgRepository_ReadBySlug_Call{Call: _e.mock.On("ReadBySlug", slug)}
}

func (_c *OrgRepository_ReadBySlug_Call) Run(run func(slug string)) *OrgRepository_ReadBySlug_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(string))
	})
	return _c
}

func (_c *OrgRepository_ReadBySlug_Call) Return(_a0 models.Org, _a1 error) *OrgRepository_ReadBySlug_Call {
	_c.Call.Return(_a0, _a1)
	return _c
}

func (_c *OrgRepository_ReadBySlug_Call) RunAndReturn(run func(string) (models.Org, error)) *OrgRepository_ReadBySlug_Call {
	_c.Call.Return(run)
	return _c
}

// Save provides a mock function with given fields: tx, t
func (_m *OrgRepository) Save(tx core.DB, t *models.Org) error {
	ret := _m.Called(tx, t)

	if len(ret) == 0 {
		panic("no return value specified for Save")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(core.DB, *models.Org) error); ok {
		r0 = rf(tx, t)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// OrgRepository_Save_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Save'
type OrgRepository_Save_Call struct {
	*mock.Call
}

// Save is a helper method to define mock.On call
//   - tx core.DB
//   - t *models.Org
func (_e *OrgRepository_Expecter) Save(tx interface{}, t interface{}) *OrgRepository_Save_Call {
	return &OrgRepository_Save_Call{Call: _e.mock.On("Save", tx, t)}
}

func (_c *OrgRepository_Save_Call) Run(run func(tx core.DB, t *models.Org)) *OrgRepository_Save_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(core.DB), args[1].(*models.Org))
	})
	return _c
}

func (_c *OrgRepository_Save_Call) Return(_a0 error) *OrgRepository_Save_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *OrgRepository_Save_Call) RunAndReturn(run func(core.DB, *models.Org) error) *OrgRepository_Save_Call {
	_c.Call.Return(run)
	return _c
}

// SaveBatch provides a mock function with given fields: tx, ts
func (_m *OrgRepository) SaveBatch(tx core.DB, ts []models.Org) error {
	ret := _m.Called(tx, ts)

	if len(ret) == 0 {
		panic("no return value specified for SaveBatch")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(core.DB, []models.Org) error); ok {
		r0 = rf(tx, ts)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// OrgRepository_SaveBatch_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'SaveBatch'
type OrgRepository_SaveBatch_Call struct {
	*mock.Call
}

// SaveBatch is a helper method to define mock.On call
//   - tx core.DB
//   - ts []models.Org
func (_e *OrgRepository_Expecter) SaveBatch(tx interface{}, ts interface{}) *OrgRepository_SaveBatch_Call {
	return &OrgRepository_SaveBatch_Call{Call: _e.mock.On("SaveBatch", tx, ts)}
}

func (_c *OrgRepository_SaveBatch_Call) Run(run func(tx core.DB, ts []models.Org)) *OrgRepository_SaveBatch_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(core.DB), args[1].([]models.Org))
	})
	return _c
}

func (_c *OrgRepository_SaveBatch_Call) Return(_a0 error) *OrgRepository_SaveBatch_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *OrgRepository_SaveBatch_Call) RunAndReturn(run func(core.DB, []models.Org) error) *OrgRepository_SaveBatch_Call {
	_c.Call.Return(run)
	return _c
}

// Transaction provides a mock function with given fields: _a0
func (_m *OrgRepository) Transaction(_a0 func(core.DB) error) error {
	ret := _m.Called(_a0)

	if len(ret) == 0 {
		panic("no return value specified for Transaction")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(func(core.DB) error) error); ok {
		r0 = rf(_a0)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// OrgRepository_Transaction_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Transaction'
type OrgRepository_Transaction_Call struct {
	*mock.Call
}

// Transaction is a helper method to define mock.On call
//   - _a0 func(core.DB) error
func (_e *OrgRepository_Expecter) Transaction(_a0 interface{}) *OrgRepository_Transaction_Call {
	return &OrgRepository_Transaction_Call{Call: _e.mock.On("Transaction", _a0)}
}

func (_c *OrgRepository_Transaction_Call) Run(run func(_a0 func(core.DB) error)) *OrgRepository_Transaction_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(func(core.DB) error))
	})
	return _c
}

func (_c *OrgRepository_Transaction_Call) Return(_a0 error) *OrgRepository_Transaction_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *OrgRepository_Transaction_Call) RunAndReturn(run func(func(core.DB) error) error) *OrgRepository_Transaction_Call {
	_c.Call.Return(run)
	return _c
}

// Update provides a mock function with given fields: tx, organization
func (_m *OrgRepository) Update(tx core.DB, organization *models.Org) error {
	ret := _m.Called(tx, organization)

	if len(ret) == 0 {
		panic("no return value specified for Update")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(core.DB, *models.Org) error); ok {
		r0 = rf(tx, organization)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// OrgRepository_Update_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Update'
type OrgRepository_Update_Call struct {
	*mock.Call
}

// Update is a helper method to define mock.On call
//   - tx core.DB
//   - organization *models.Org
func (_e *OrgRepository_Expecter) Update(tx interface{}, organization interface{}) *OrgRepository_Update_Call {
	return &OrgRepository_Update_Call{Call: _e.mock.On("Update", tx, organization)}
}

func (_c *OrgRepository_Update_Call) Run(run func(tx core.DB, organization *models.Org)) *OrgRepository_Update_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(core.DB), args[1].(*models.Org))
	})
	return _c
}

func (_c *OrgRepository_Update_Call) Return(_a0 error) *OrgRepository_Update_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *OrgRepository_Update_Call) RunAndReturn(run func(core.DB, *models.Org) error) *OrgRepository_Update_Call {
	_c.Call.Return(run)
	return _c
}

// NewOrgRepository creates a new instance of OrgRepository. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewOrgRepository(t interface {
	mock.TestingT
	Cleanup(func())
}) *OrgRepository {
	mock := &OrgRepository{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
