// Code generated by mockery v2.46.2. DO NOT EDIT.

package mocks

import mock "github.com/stretchr/testify/mock"

// VulndbConfigService is an autogenerated mock type for the configService type
type VulndbConfigService struct {
	mock.Mock
}

type VulndbConfigService_Expecter struct {
	mock *mock.Mock
}

func (_m *VulndbConfigService) EXPECT() *VulndbConfigService_Expecter {
	return &VulndbConfigService_Expecter{mock: &_m.Mock}
}

// GetJSONConfig provides a mock function with given fields: key, v
func (_m *VulndbConfigService) GetJSONConfig(key string, v any) error {
	ret := _m.Called(key, v)

	if len(ret) == 0 {
		panic("no return value specified for GetJSONConfig")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(string, any) error); ok {
		r0 = rf(key, v)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// VulndbConfigService_GetJSONConfig_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'GetJSONConfig'
type VulndbConfigService_GetJSONConfig_Call struct {
	*mock.Call
}

// GetJSONConfig is a helper method to define mock.On call
//   - key string
//   - v any
func (_e *VulndbConfigService_Expecter) GetJSONConfig(key interface{}, v interface{}) *VulndbConfigService_GetJSONConfig_Call {
	return &VulndbConfigService_GetJSONConfig_Call{Call: _e.mock.On("GetJSONConfig", key, v)}
}

func (_c *VulndbConfigService_GetJSONConfig_Call) Run(run func(key string, v any)) *VulndbConfigService_GetJSONConfig_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(string), args[1].(any))
	})
	return _c
}

func (_c *VulndbConfigService_GetJSONConfig_Call) Return(_a0 error) *VulndbConfigService_GetJSONConfig_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *VulndbConfigService_GetJSONConfig_Call) RunAndReturn(run func(string, any) error) *VulndbConfigService_GetJSONConfig_Call {
	_c.Call.Return(run)
	return _c
}

// SetJSONConfig provides a mock function with given fields: key, v
func (_m *VulndbConfigService) SetJSONConfig(key string, v any) error {
	ret := _m.Called(key, v)

	if len(ret) == 0 {
		panic("no return value specified for SetJSONConfig")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(string, any) error); ok {
		r0 = rf(key, v)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// VulndbConfigService_SetJSONConfig_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'SetJSONConfig'
type VulndbConfigService_SetJSONConfig_Call struct {
	*mock.Call
}

// SetJSONConfig is a helper method to define mock.On call
//   - key string
//   - v any
func (_e *VulndbConfigService_Expecter) SetJSONConfig(key interface{}, v interface{}) *VulndbConfigService_SetJSONConfig_Call {
	return &VulndbConfigService_SetJSONConfig_Call{Call: _e.mock.On("SetJSONConfig", key, v)}
}

func (_c *VulndbConfigService_SetJSONConfig_Call) Run(run func(key string, v any)) *VulndbConfigService_SetJSONConfig_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(string), args[1].(any))
	})
	return _c
}

func (_c *VulndbConfigService_SetJSONConfig_Call) Return(_a0 error) *VulndbConfigService_SetJSONConfig_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *VulndbConfigService_SetJSONConfig_Call) RunAndReturn(run func(string, any) error) *VulndbConfigService_SetJSONConfig_Call {
	_c.Call.Return(run)
	return _c
}

// NewVulndbConfigService creates a new instance of VulndbConfigService. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewVulndbConfigService(t interface {
	mock.TestingT
	Cleanup(func())
}) *VulndbConfigService {
	mock := &VulndbConfigService{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
