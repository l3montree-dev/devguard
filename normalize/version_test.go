...
func TestConvertRPMtoSemVer(t *testing.T) {
	...
	t.Run("version with only release number", func(t *testing.T) {
		...
		_, err := normalize.ConvertRPMtoSemVer("15.fc39")
		assert.Error(t, err)
	})

	t.Run("version with no numeric release", func(t *testing.T) {
		result, err := normalize.ConvertRPMtoSemVer("1.2.3-beta")
		assert.NoError(t, err)
		assert.Equal(t, "1.2.3-beta", result)
	})

	t.Run("empty string", func(t *testing.T) {
		_, err := normalize.ConvertRPMtoSemVer("")
		assert.Error(t, err)
	})

	t.Run("complex version with letters and epoch", func(t *testing.T) {
		_, err := normalize.ConvertRPMtoSemVer("0:B.02.16-4.el6cf")
		assert.Error(t, err)
	})

	t.Run("version with multiple hyphens", func(t *testing.T) {
		_, err := normalize.ConvertRPMtoSemVer("2024.2.69_v8.0.303-91.4.el9_4")
		assert.Error(t, err)
	})
}
