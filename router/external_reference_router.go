...
func NewExternalReferenceRouter(
	...
	refWriteGroup.POST("/", externalReferenceController.Create, middlewares.AssetAccessControl(shared.ObjectAsset, shared.ActionUpdate))        // Create reference
	refWriteGroup.POST("/sync/", externalReferenceController.Sync, middlewares.AssetAccessControl(shared.ObjectAsset, shared.ActionUpdate))     // Sync external sources
	refWriteGroup.DELETE("/:url/", externalReferenceController.Delete, middlewares.AssetAccessControl(shared.ObjectAsset, shared.ActionUpdate)) // Delete reference by URL-encoded URL
