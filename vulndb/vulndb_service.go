...
const vulnDBURI = "ghcr.io/l3montree-dev/devguard/vulndb/v4.1"
...
func (service *VulnDBService) ImportRC(ctx context.Context, opts shared.ImportOptions) (err error) {
	...
	if err != nil {
		...
		monitoring.Alert("vulndb integrity check failed", err, monitoring.AlertOptions{Ctx: ctx})
		span.SetAttributes(
...
func TruncateCVERelatedTables(ctx context.Context, tx pgx.Tx) error {
	// truncate all tables to ensure no data inconsistencies before running the export command
...
func (service *VulnDBService) applyFromWorkingDir(ctx context.Context, tx pgx.Tx, workingDir string, integrityGroundTruth IntegrityInformation, bulk bool, tryQuickDiff bool) ([]string, error) {
	if tryQuickDiff {
		...
		if err != nil {
			...
			monitoring.Alert("vulndb quick-diff apply failed", fmt.Errorf("quick-diff apply failed: %w, will retry as full sync", err), monitoring.AlertOptions{Ctx: ctx})
