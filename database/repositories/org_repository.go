...
func (g *orgRepository) List(ctx context.Context, tx *gorm.DB, ids []uuid.UUID) ([]models.Org, error) {
	var ts []models.Org
	err := g.GetDB(ctx, tx).Model(models.Org{}).Preload("GithubAppInstallations").Where("id = Any ?", ids).Find(&ts).Error
	return ts, err
}
...
func (g *orgRepository) ContentTree(ctx context.Context, tx *gorm.DB, orgID uuid.UUID, projects []string) []any {
	...
	g.GetDB(ctx, tx).Preload("Assets").Where(`projects.id = Any ? AND projects.organization_id = ?`, projects, orgID).Find(&projectModels)
...
func (g *orgRepository) GetOrgByIDs(ctx context.Context, tx *gorm.DB, ids []uuid.UUID) ([]models.Org, error) {
	var orgs []models.Org
	err := g.GetDB(ctx, tx).Model(models.Org{}).Where("id = Any ?", ids).Find(&orgs).Error
	return orgs, err
}
