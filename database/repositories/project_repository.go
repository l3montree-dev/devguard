...
func (g *projectRepository) GetByProjectIDs(ctx context.Context, tx *gorm.DB, projectIDs []uuid.UUID) ([]models.Project, error) {
	var projects []models.Project
	err := g.GetDB(ctx, tx).Model(&models.Project{}).Where("ID = Any ?", projectIDs).Find(&projects).Error
	return projects, err
}
...
func (g *projectRepository) ListSubProjectsAndAssets(
	...
	if len(allowedAssetIDs) > 0 {
		assetQuery = assetQuery.Where("id = Any ? OR is_public = true", allowedAssetIDs)
	} else {
	...
	if len(allowedProjectIDs) > 0 {
		projectQuery = projectQuery.Where("id = Any ? OR (organization_id = ? AND is_public = true)", allowedProjectIDs, orgID)
	} else {
...
func (g *projectRepository) ListPaged(ctx context.Context, tx *gorm.DB, projectIDs []uuid.UUID, parentID *uuid.UUID, orgID uuid.UUID, pageInfo shared.PageInfo, search string, filter []shared.FilterQuery, sort []shared.SortQuery) (shared.Paged[models.Project], error) {
	...
	if parentID != nil {
		q = g.GetDB(ctx, tx).Model(&models.Project{}).Where(
			g.GetDB(ctx, tx).Where("id = Any ? AND parent_id = ?", projectIDs, parentID).
				Or("organization_id = ? AND is_public = true AND parent_id = ?", orgID, parentID),
		)
	} else {
		q = g.GetDB(ctx, tx).Model(&models.Project{}).Where(
			g.GetDB(ctx, tx).Where("id = Any ? AND parent_id IS NULL", projectIDs).
...
func (g *projectRepository) List(ctx context.Context, tx *gorm.DB, projectIDs []uuid.UUID, parentID *uuid.UUID, orgID uuid.UUID) ([]models.Project, error) {
	var projects []models.Project
	if parentID != nil {
		err := g.GetDB(ctx, tx).Where("id = Any ? AND parent_id = ?", projectIDs, parentID).Or("organization_id = ? AND is_public = true AND parent_id = ?", orgID, parentID).Find(&projects).Error
		return projects, err
	}
	err := g.GetDB(ctx, tx).Where("id = Any ? AND parent_id IS NULL", projectIDs).Or("organization_id = ? AND is_public = true AND parent_id IS NULL", orgID).Find(&projects).Error
	return projects, err
}
...
func (g *projectRepository) GetChildProjectsForParents(ctx context.Context, tx *gorm.DB, parentIDs []uuid.UUID, providerID string) ([]models.Project, error) {
	var projects []models.Project
	err := g.GetDB(ctx, tx).Where("parent_id = Any ? AND external_entity_provider_id = ?", parentIDs, providerID).Find(&projects).Error
	return projects, err
}
...
func (g *projectRepository) UpsertSplit(ctx context.Context, tx *gorm.DB, externalProviderID string, projects []*models.Project) ([]*models.Project, []*models.Project, error) {
	...
	err := g.GetDB(ctx, tx).Where("external_entity_id = Any ? AND external_entity_provider_id = ?", utils.Map(projects, func(p *models.Project) *string { return p.ExternalEntityID }), externalProviderID).Find(&existingProjects).Error
	if err != nil {
		return nil, nil, err
	}
