...
func (i *JiraIntegration) HandleEvent(ctx context.Context, event any, userAgent *string) error {
	...
	case shared.ManualMitigateEvent:
		...
		return i.CreateIssue(event.Ctx.Request().Context(), asset, assetVersionName, vuln, projectSlug, orgSlug, event.Justification, session.GetOwnerID(), userAgent)
		...
		case dtos.EventTypeReopened:
			...
			if err != nil {
				if err.Error() == `failed to create issue comment, status code: 404, response: {"errorMessages":["Issue does not exist or you do not have permission to see it."],"errors":{}}` {
					...
					session := shared.GetSession(event.Ctx)
					err = i.CreateIssue(event.Ctx.Request().Context(), asset, assetVersionName, vuln, projectSlug, orgSlug, utils.SafeDereference(ev.Justification), session.GetOwnerID(), userAgent)
					if err != nil {
