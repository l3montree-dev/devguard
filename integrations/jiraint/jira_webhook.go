...
func (i *JiraIntegration) HandleWebhook(ctx shared.Context) error {
	...
	case EventIssueUpdated:
		...
		case StatusCategoryDone:
			...
			vulnEvent = models.NewAcceptedEvent(vuln.GetID(), vuln.GetType(), fmt.Sprintf("jira:%s", userID), fmt.Sprintf("This Vulnerability is marked as accepted by %s, due to closing of the jira ticket.", username), false, &userAgent)
		...
		case StatusCategoryToDo, StatusCategoryInProgress:
			...
			vulnEvent = models.NewReopenedEvent(vuln.GetID(), vuln.GetType(), fmt.Sprintf("jira:%s", userID), fmt.Sprintf("This Vulnerability was reopened by %s", username), false, &userAgent)
