...
// When the same CVE+path is open on more than one asset version at the
// moment a VEX rule is created, the rule must close every one of them, not
// just one representative.
func TestVEXRuleClosesAllOpenVulnsWithSameCVEAndPath(t *testing.T) {
	t.Parallel()
	WithTestApp(t, "../initdb.sql", func(f *TestFixture) {
		...
		// Both branches are scanned BEFORE the rule exists, so both vulns
		// are open at once and share the same cve_id + vulnerability_path.
		...
		assert.Equal(t, dtos.VulnStateOpen, fetchVuln(featureBranch).State)
		...
		assert.Equal(t, dtos.VulnStateFalsePositive, fetchVuln(defaultBranch).State,
			"vuln on the default branch must be closed")
		assert.Equal(t, dtos.VulnStateFalsePositive, fetchVuln(featureBranch).State,
			"identical vuln on the feature branch must be closed too, even though both were open at rule-creation time")
	})
}
