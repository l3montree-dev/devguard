...
func TestGitlabWebhookHandleWebhook(t *testing.T) {
	...
	WithTestAppOptions(t, "../initdb.sql", TestAppOptions{
		...
		assert.Nil(t, f.DB.Create(&integration).Error)

		// connect the asset to gitlab
		asset.RepositoryID = new("gitlab:" + integration.ID.String() + ":1")
		...
		t.Run("should validate the webhook secret token and return an error if it doesnt match", func(t *testing.T) {
			...
			asset.WebhookSecret = new(uuid.New())
			f.DB.Save(&asset)
		...
		t.Run("should reopen the ticket, if the action is reopen and the vuln isnt open", func(t *testing.T) {
			...
			assert.Nil(t, f.DB.Save(&vuln).Error)
		...
		t.Run("should regenerate risk history when vulnerability state changes via comment", func(t *testing.T) {
			...
			assert.Nil(t, f.DB.Model(&vuln).Association("Artifacts").Append(&artifact))
			...
			assert.Nil(t, f.DB.Create(&artifactRiskHistory).Error) // Test a comment event that changes vulnerability state to accepted
			...
			err = f.DB.Where("artifact_name = ? AND asset_version_name = ? AND asset_id = ? AND day = CURRENT_DATE",
			...
			vulnFromDB := models.DependencyVuln{}
			assert.Nil(t, f.DB.Preload("Artifacts").First(&vulnFromDB, "id = ?", vuln.ID).Error)
			assert.Equal(t, dtos.VulnStateAccepted, vulnFromDB.State)
