...
func (s *projectService) SearchProjectsWithSubProjectsAndAssetsPaged(c shared.Context) (shared.Paged[dtos.ProjectDTO], error) {
	...
	allowedProjectIDs, err := rbac.GetAllProjectsForUser(shared.GetSession(c).GetOwnerID())
	if err != nil {
...
func (s *projectService) ListAllowedProjectsPaged(c shared.Context) (shared.Paged[models.Project], error) {
	...
	rbac := shared.GetRBAC(c)
	projectIDs, err := rbac.GetAllProjectsForUser(shared.GetSession(c).GetOwnerID())
	if err != nil {
