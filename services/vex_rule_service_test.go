...
func TestVEXRuleSetPathPattern(t *testing.T) {
	rule := &models.VEXRule{}

	pattern := []string{"pkg:npm/mylib@1.0.0", "**", "*.txt"}
	rule.SetPathPattern(pattern)

	assert.Equal(t, pattern, rule.PathPattern)
	assert.NotEmpty(t, rule.PathPatternHash)

	// Verify same pattern produces same hash
	hash1 := rule.PathPatternHash
	rule.SetPathPattern(pattern)
	assert.Equal(t, hash1, rule.PathPatternHash)

	// Verify different pattern produces different hash
	rule.SetPathPattern([]string{"different", "pattern"})
	assert.NotEqual(t, hash1, rule.PathPatternHash)
}

func TestVEXRuleCompositeKey(t *testing.T) {
	assetID := uuid.New()
	...
	rule.SetPathPattern([]string{"path", "to", "component"})

	// Verify composite key fields are set
	assert.Equal(t, assetID, rule.AssetID)
	assert.Equal(t, "CVE-2024-1234", rule.CVEID)
	assert.Equal(t, "test-source", rule.VexSource)
	assert.NotEmpty(t, rule.PathPatternHash)

	// Verify hash is deterministic
	expectedHash := models.CalculatePathPatternHash([]string{"path", "to", "component"})
	assert.Equal(t, expectedHash, rule.PathPatternHash)
}
